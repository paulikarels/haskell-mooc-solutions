...
greetText test 
    | T.length test > 15 = T.pack "Hello, " <> T.take 15 test <> T.pack "...!" 
    | otherwise = T.pack "Hello, " <> test <> T.pack "!" 
...
shout :: T.Text -> T.Text
shout test = T.unwords (zipWith ($) (cycle [T.toUpper, id]) (T.words test))
...
longestRepeat test = helper 0 (T.unpack test)
    where
        helper x [] = x
        helper x (y : ys) = helper (max x (length (takeWhile (== y) (y:ys)))) (dropWhile (==y) ys)
...
takeStrict x y = TL.toStrict (TL.take x y)
...
countUtf8Chars x = case decodeUtf8' x of
