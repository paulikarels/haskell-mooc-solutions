...
split str = do
  s <- elemIndex ' ' str
  return (take s str, drop (s + 1) str)
...
checkNumber (x, y) = if not (any isDigit x) && not (any isDigit y) then Just (x, y) else Nothing
...
winner scores player1 player2 = 
  if lookup player1 scores == Nothing || lookup player2 scores == Nothing then Nothing else 
    if lookup player1 scores >= lookup player2 scores then Just player1 else Just player2

-- (lookup player1 scores > lookup player2 scores then Just player1 else Just player2)
------------------------------------------------------------------------------
...
balance accountName = BankOp bank
  where
    bank (Bank x) = (Map.findWithDefault 0 accountName x, Bank x)
