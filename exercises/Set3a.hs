...
joinToLength x sx = [i | first <- sx, last <- sx, let i = first ++ last, length i == x ]
...
interpreter commands = interpreterHelper commands 0 0 
  where
    ...
    interpreterHelper ("printX":commands) x y   = show x : interpreterHelper commands x y 
    interpreterHelper ("printY":commands) x y   = show y : interpreterHelper commands x y 
    interpreterHelper ("up":commands) x y       = interpreterHelper commands x (y+1) 
    interpreterHelper ("down":commands) x y     = interpreterHelper commands x (y-1) 
    interpreterHelper ("right":commands) x y    = interpreterHelper commands (x+1) y
    interpreterHelper ("left":commands) x y     = interpreterHelper commands (x-1) y 
