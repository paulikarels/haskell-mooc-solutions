...
--   treeMax (Node 6 Empty Empty) ==> 6
--   treeMax (Node 1 (Node 11 (Node 11 Empty Empty) (Node 13 Empty Empty)) (Node 7 (Node 0 Empty Empty) (Node 20 Empty Empty))) ==> 20
treeMax :: Tree Int -> Int
treeMax Empty        = 0
treeMax (Node a l r) = (max (treeMax l) (max a (treeMax r)))
...
allValues condition (Node a l r) 
  | condition a          = allValues condition l && allValues condition r 
  | otherwise            = False
...
isOrdered (Node a l r) 
  | root && subs = True
  | otherwise        = False
    where   root = (aV (<a) l) && (aV (>a) r)
              where aV condition tree = allValues condition tree
            subs = isOrdered l && isOrdered r
