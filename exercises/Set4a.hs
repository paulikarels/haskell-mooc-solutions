...
winner scores player1 player2 
    | (x player1) < (x player2) = player2
    | otherwise                 = player1
    where x v = Map.findWithDefault 0 v scores
...
transfer from to amount bank 
    ...
    | otherwise                                  = balance to (take from bank)
                                                                  where balance = Map.adjust (\x -> x + amount)
                                                                        take    = Map.adjust (\x -> x - amount)  
...
maxIndex array = fst ((sortBy getMaxValue (assocs array)) !! 0)
                  where getMaxValue x y = compare (snd y) (snd x)
