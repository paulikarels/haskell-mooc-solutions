...
data State = Start | Error | Finished | AddE | AddF | AddS | AddFandS | Mixed 
  deriving (Eq,Show)

step :: State -> Event -> State
step Start AddEggs      = AddE
step AddE AddFlour      = AddF
step AddE AddSugar      = AddS
step AddF AddSugar      = AddFandS
step AddFandS Mix       = Mixed
step AddS AddFlour      = AddFandS
...
passwordAllowed psw (MinimumLength n)  = length psw >= n
passwordAllowed psw (ContainsSome x)   = any (\c -> elem c x) psw
passwordAllowed psw (DoesNotContain x) = not (passwordAllowed psw (ContainsSome x))
passwordAllowed psw (And x y)          = passwordAllowed psw x && passwordAllowed psw y
passwordAllowed psw (Or x y)           = passwordAllowed psw x || passwordAllowed psw y
